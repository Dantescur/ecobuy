...
import { ProductsQuantitySelector } from '#components'
...
const addToCart = async () => {
  ...
  try {
    ...
    quantity.value = 0
    setTimeout(() => quantity.value = 1, 150)
...
<template>
  <div class="space-y-4 md:space-y-6">
    ...
    <ProductsQuantitySelector :product-id="product.id" v-model:quantity="quantity" />
    <AddToCartButton :is-adding="isAdding" :show-added="showAdded" :price="product.price" :quantity="quantity"
      @click="addToCart" />
    <ProductExtras />
  </div>
</template>
